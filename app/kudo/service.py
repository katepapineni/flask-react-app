...
class Service(object):
    ...
    def __init__(self, user_id,
                 repo_client=Repository(adapter=MongoRepository)):
    ...
    def find_all_kudos(self):
        ...
        logging.info(f'this is get{kudos}')
        return [self.dump(kudo) for kudo in kudos]

    def find_kudo(self, repo_id):
        kudo = self.repo_client.find({'user_id': self.user_id,
                                      'id': repo_id})
    ...
    def update_kudo_with(self, repo_id, githubRepo):
        records_affected = self.repo_client.update(
            {'user_id': self.user_id, 'id': repo_id},
